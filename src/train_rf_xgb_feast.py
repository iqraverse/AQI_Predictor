...
# -------- 7️⃣ Initialize MLflow (SQLite Backend for Windows) --------
...
sns.scatterplot(ax=axes[0], x="Actual", y="Predicted", data=rf_df,
                color="forestgreen", alpha=0.6, s=40, label="Predicted (RF)")
sns.lineplot(ax=axes[0], x=[y_test.min(), y_test.max()],
             y=[y_test.min(), y_test.max()], color="red", lw=2, label="Perfect Fit")
axes[0].set_title(f"Random Forest (R²={rf_r2:.3f})", fontsize=13, weight="bold")
...
sns.scatterplot(ax=axes[1], x="Actual", y="Predicted", data=xgb_df,
                color="royalblue", alpha=0.6, s=40, label="Predicted (XGB)")
sns.lineplot(ax=axes[1], x=[y_test.min(), y_test.max()],
             y=[y_test.min(), y_test.max()], color="red", lw=2, label="Perfect Fit")
axes[1].set_title(f"XGBoost (R²={xgb_r2:.3f})", fontsize=13, weight="bold")
axes[1].set_xlabel("Actual AQI"); axes[1].set_ylabel("Predicted AQI")
axes[1].legend(); axes[1].grid(True, alpha=0.3)

plt.suptitle("📈 Actual vs Predicted AQI Comparison", fontsize=15, weight="bold")
...
print(f"🗃️ MLflow DB stored at: {MLFLOW_DB_PATH}")
print("\n🏁 Model training + registration completed successfully using Feast + MLflow (SQLite) 🚀")
